...
class MSDeformAttnPixelDecoder(nn.Module):
    def __init__(
        ...
        common_stride=4,
        backbone_out_features=["res2", "res3", "res4", "res5"],
        ...
        if not input_shape:
            input_shape = dict()
            for name, ch, stride in zip(
                backbone_out_features, backbone_out_channels, backbone_out_strides
            ):
                input_shape[name] = dict({"channel": ch, "stride": stride})

        print("transformer_in_features", transformer_in_features)
        ...
        print("transformer_in_features", self.transformer_in_features)
        ...
        print("num_fpn_levels", self.num_fpn_levels)
    ...
    def override_input_shape(self, input_shape):
        ...
        self.__init__(
            ...
            transformer_enc_layers=self.transformer_enc_layers,
            conv_dim=self.conv_dim,
            mask_dim=self.mask_dim,
            norm=self.norm,
            transformer_in_features=self.transformer_in_features,

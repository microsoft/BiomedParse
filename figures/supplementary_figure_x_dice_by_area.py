...
eval_results_path = os.path.join(base_dir, 'biomedparse_eval_results.json')
...
df = merge_with_sam_medsam(df, parsed_data, base_dir)

# Save to CSV
df.to_csv(os.path.join(base_dir, 'dice_by_size.csv'), index=False)
...
def plot_area_to_dice(df):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, 'area_vs_dice.pdf'), dpi=300)
    plt.show()
